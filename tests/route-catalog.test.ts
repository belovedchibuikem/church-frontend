...
test('the approved contact sheets map to 258 unique reference screens', () => {
  assert.equal(adminScreens.length, 258);
  assert.equal(new Set(adminScreens.map((screen) => screen.route)).size, 258);
  ...
  assert.equal(adminScreens.filter((screen) => screen.batch === 'L').length, 17);
