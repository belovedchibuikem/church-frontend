...
export function AdminActionSurface({ mode, label, pageTitle, permission, scope, entityKey, record, details = {}, items = [], records = [], onClose, onSubmit }: Props) {
  ...
  if (mode === 'file') return <form className="interaction-action-form" onSubmit={save}>
    ...
    {errorNote ?? <div className="interaction-safety-note">{t('admin.exportSafetyNote', { defaultMessage: 'File generation needs an authorized document operation. This does not fabricate a file.' })}</div>}
      ...
      <form className="interaction-action-form" onSubmit={save}>
        ...
        {errorNote ?? (
          <div className="interaction-safety-note">
            {t('admin.confirmSafetyNote', { defaultMessage: 'Confirm submits the matching platform operation. Failures are shown here; success is never simulated.' })}
          </div>
        )}
        {footerButtons(
          /delete|remove/i.test(label)
